...
The levered model was ported from an Excel-based project-finance workbook used for real deal appraisal, simplified down to what's useful in a scenario-exploration tool (the original scenario-sweep tables, IRR heatmaps, working capital and terminal value were all dropped). It originally reproduced the source workbook's Project IRR, Equity IRR and gearing within about 0.3 percentage points; that benchmark no longer holds exactly, since devex timing was deliberately changed from the source workbook's multi-year development spend to a lump sum at FID (see below) — a methodology improvement, not a bug.
...
`ProjectFinanceInputs` holds everything else: build cost, connection cost and devex per technology, fixed O&M, the FID period and construction timeline, PPA tenor and tariff, penalty multiple, indexation rates (cost, PPA, and solar/non-solar merchant prices separately), debt tenor and rate, DSCR targets and gearing caps for the contracted and uncontracted revenue tranches, depreciation rates, and the corporate tax rate. Most of these seed from scenario defaults but are fully editable in the Financial Model tab.
...
1. **Timeline.** FID (`fid_period`) is when devex is paid and construction begins. Construction periods per technology are back-aligned so every technology finishes at the same point, then the operating life and PPA tenor are laid out from there. There is no separate multi-year development phase.
2. **Capital spend.** Devex is the expected cost of reaching FID: a single lump sum per technology (wind/PV/BESS), paid in the FID period and funded 100% by equity (never refinanced by debt). Capex is spread evenly across each technology's own construction window, indexed by cost inflation. Both are also broken out per technology in the schedule (e.g. `devex_onsw`, `capex_pv`), not just as combined totals.
3. **Generation, revenue and opex.** Revenue is broken out by type — volume × price = revenue for PPA delivery, penalty, merchant solar, merchant non-solar, and LGC/GO — summing to total revenue; merchant volumes come from the excess volumes during the PPA term and from total generation once it expires. Opex is fixed O&M plus a percentage-of-revenue ancillary cost.
4. **Debt sizing.** Cash available for debt service is split into contracted and uncontracted tranches by revenue share. Each tranche's debt capacity is the smaller of what a gearing cap and a DSCR target will support, discounted back at the debt rate. Debt draws down from financial close (= FID) against capex only — devex is never debt-funded — with interest during construction accruing on the closing balance and capitalizing into the total debt. This is solved iteratively, since the drawdown and the interest on it are circular.
5. **Depreciation.** Straight-line, book and tax bases tracked separately: devex is capitalized into the book base (depreciated over the operating life, like capex) but fully expensed for tax in the period it's paid; capitalized interest during construction is also part of the book base only.
...
9. **Balance sheet & cash flow statement.** A full balance sheet (PP&E net, cash, debt, share capital, retained earnings) and a three-section cash flow statement (operating/investing/financing) roll forward every period, with a balance check (assets − liabilities − equity, which should read ~0) surfaced in both the app and the Excel export. No dividends or distributions are modeled yet: all profit and cash generated is retained, so retained earnings and the cash balance simply accumulate.
10. **LCOE.** Annualized capital cost plus fixed O&M, divided by annual generation.

## Excel export

The Financial Model tab can export a live workbook, not just a snapshot of numbers. Revenue (by type), opex, EBITDA, capex and devex (by technology), depreciation, tax, cash flow, and the full balance sheet (including its balance check) are all written as actual Excel formulas that recompute if you change an input cell. Debt drawdown and interest during construction are the one exception: because debt sizing is circular, they're written as pre-solved values with a note on the workbook's Notes sheet explaining that changing a cost assumption means re-running the toolkit to re-size debt, rather than expecting Excel to solve the circularity itself.
