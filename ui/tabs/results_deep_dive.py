...
from ppa.results import build_supply_mix_df
...
def _render_dispatch_section(result, s, chosen_day: str) -> None:
    ...
    with tab_chart2:
        if getattr(result, "market_prices", None) is not None:
            ...
            price_day = result.market_prices[
                result.market_prices.index.strftime("%Y-%m-%d") == chosen_day
            ]
            fig_price = make_price_series_chart(
                price_day, title=f"Day-ahead price: {chosen_day}"
            )
...
def _render_gen_stats(result, s) -> None:
    ...
    wind_cf = (
        summary.wind_generation_mwh / (s.onsw_mw * n_hours) if s.onsw_mw > 0 else 0.0
    )
    pv_cf = summary.pv_generation_mwh / (s.pv_mw * n_hours) if s.pv_mw > 0 else 0.0
    bess_cycles = (
        summary.bess_dispatch_mwh / s.effective_bess_mwh
        if s.include_bess and s.effective_bess_mwh > 0
        else 0.0
    )
    ...
    stats_df = pd.DataFrame(
        [
            (
                ...
                f"{wind_cf:.1%}",
                f"{summary.wind_generation_mwh:,.0f} MWh",
            ),
            (
                "PV capacity factor",
                f"{pv_cf:.1%}",
                f"{summary.pv_generation_mwh:,.0f} MWh",
            ),
            ("BESS equivalent cycles", f"{bess_cycles:.1f}", f"over {n_hours} h"),
            (
                "BESS average SoC",
                f"{avg_soc:.1f} MWh",
                f"{avg_soc / s.effective_bess_mwh:.1%} of capacity"
                if s.effective_bess_mwh > 0
                else "",
            ),
...
def _render_multi_year_counterfactuals(results, fin, s) -> None:
    ...
    st.plotly_chart(
        make_multi_year_counterfactual_chart(
            years, ppa_prices, spot_prices, cal_prices, blended_prices
        ),
        width="stretch",
    )
    ...
    def _eff(cost):
        return cost / total_load if total_load > 0 else 0.0
...
def _render_multi_year_deep_dive() -> None:
        ...
        with cols[0]:
            ...
            capex_df = pd.DataFrame(
                [
                    (
                        "Onshore wind",
                        _fmt_m(fin.capex.capex_wind),
                        f"{s.onsw_mw:.0f} MW × €{s.wind_capex_per_kw:,.0f}/kW",
                    ),
                    (
                        "Solar PV",
                        _fmt_m(fin.capex.capex_pv),
                        f"{s.pv_mw:.0f} MW × €{s.pv_capex_per_kw:,.0f}/kW",
                    ),
                    (
                        "BESS",
                        _fmt_m(fin.capex.capex_bess),
                        f"{s.effective_bess_mwh:.0f} MWh × €{s.bess_capex_per_kwh:,.0f}/kWh",
                    ),
                    ("Total CAPEX", _fmt_m(fin.capex.capex_total), ""),
                    (
                        "Annual OPEX",
                        _fmt_m(fin.annual_opex),
                        f"{s.opex_rate:.0%} of CAPEX",
                    ),
                ],
            ...
            econ_df = pd.DataFrame(
                [
                    ...
                    (
                        ...
                        f"{fin.total_lifetime_generation_mwh / 1e3:.0f} GWh",
                        "",
                    ),
                ],
    ...
    cols = st.columns(4)
    selected_year = cols[0].selectbox(
        "**Year to analyze:**", year_options, key="dd_year"
    )
    year_idx = year_options.index(selected_year)
    result = results[year_idx]

    available_days = sorted(
        result.dispatch.wind_gen.index.normalize().unique().strftime("%Y-%m-%d")
    )
    ...
    with st.expander(f"Financial summary for {selected_year}", expanded=False):
        ...
        cols[4].metric(
            "Wind+PV Gen", f"{(yf.wind_gen_mwh + yf.pv_gen_mwh) / 1e3:.0f} GWh"
        )
    ...
    with st.expander("Detailed results", expanded=False):
        ...
        with tab_chart1:
            ...
            chosen_day = cols[0].selectbox(
                "Day to inspect", available_days, index=0, key="dd_chosen_day1"
            )
...
def _render_single_day_deep_dive() -> None:
    ...
    if fin is None:
        ...
        with cols[1]:
            st.markdown("**Project economics**")
            irr_str = (
                f"{fin.project_irr:.1%}" if not np.isnan(fin.project_irr) else "n/a"
            )
            lcoe_str = f"€{fin.lcoe:.2f}/MWh" if not np.isnan(fin.lcoe) else "n/a"
            be_str = (
                f"€{fin.breakeven_ppa_price:.2f}/MWh"
                if not np.isnan(fin.breakeven_ppa_price)
                else "n/a"
            )
            econ_df = pd.DataFrame(
                [
                    ...
                    (
                        ...
                        f"{fin.annual_gen_mwh:,.0f} MWh",
                        "",
                    ),
                    (
                        "Annual PPA revenue",
                        _fmt_m(fin.annual_ppa_rev),
                        f"€{s.ppa_price:.0f}/MWh",
                    ),
                    (
                        "Annual merchant revenue",
                        _fmt_m(fin.annual_merch_rev),
                        f"avg €{fin.avg_merch_price:.2f}/MWh",
                    ),
                    (
                        "Annual market purchase cost",
                        _fmt_m(fin.annual_buy_cost),
                        f"avg €{fin.avg_buy_price:.2f}/MWh",
                    ),
                    ...
                    (
                        "NPV at WACC",
                        _fmt_m(fin.npv_at_wacc),
                        f"at {s.discount_rate:.0%}",
                    ),
                    (
                        f"Breakeven PPA for {s.target_irr:.0%} IRR",
                        be_str,
                        f"vs €{s.ppa_price:.0f}/MWh contracted",
                    ),
                ],
                columns=["Metric", "Value", "Note"],
            )
    ...
    if ts is not None:
        ...
        default_idx = (
            available_days.index(s.chosen_day) if s.chosen_day in available_days else 14
        )
        ...
        if getattr(result, "market_prices", None) is not None:
            fig_price = make_price_series_chart(
                result.market_prices, title="Market spot price"
            )
        else:
