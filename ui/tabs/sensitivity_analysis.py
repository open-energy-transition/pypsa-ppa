...
def _what_if_panel(
    base_energy: EnergyInputs, base_finance: ProjectFinanceInputs
) -> None:
    with st.expander("What-if analysis", expanded=False):
        ...
        with cols[1]:
            st.markdown("**Revenue**")
            tariff = _num(
                "PPA tariff (€/MWh)", pf + "tariff", base_finance.ppa_tariff, step=1.0
            )
            pen = _num(
                "Penalty multiple (×)",
                pf + "pen",
                base_finance.penalty_multiple,
                step=0.1,
                fmt="%.2f",
            )
            ...
            solar_infl = _num(
                "Solar price",
                pf + "solar_infl",
                base_finance.solar_price_inflation,
                step=0.1,
                fmt="%.2f",
                pct=True,
            )
            nonsolar_infl = _num(
                "Non-solar price",
                pf + "nonsolar_infl",
                base_finance.nonsolar_price_inflation,
                step=0.1,
                fmt="%.2f",
                pct=True,
            )
        ...
        with cols[2]:
            ...
            debt_rate = _num(
                "Debt rate (%)",
                pf + "debt_rate",
                base_finance.debt_rate,
                step=0.1,
                fmt="%.2f",
                pct=True,
            )
            debt_tenor = int(
                _num("Tenor (yrs)", pf + "debt_tenor", base_finance.debt_tenor, step=1)
            )
            dscr_c = _num(
                "DSCR contracted",
                pf + "dscr_c",
                base_finance.dscr_contracted,
                step=0.05,
                fmt="%.2f",
            )
            dscr_u = _num(
                "DSCR uncontracted",
                pf + "dscr_u",
                base_finance.dscr_uncontracted,
                step=0.05,
                fmt="%.2f",
            )
            gear_c = _num(
                "Max gearing contr. (%)",
                pf + "gear_c",
                base_finance.max_gearing_contracted,
                step=1.0,
                fmt="%.1f",
                pct=True,
            )
            gear_u = _num(
                "Max gearing uncontr. (%)",
                pf + "gear_u",
                base_finance.max_gearing_uncontracted,
                step=1.0,
                fmt="%.1f",
                pct=True,
            )
        ...
        with cols[3]:
            ...
            book_dep = _num(
                "Book dep. rate (%)",
                pf + "book_dep",
                base_finance.book_depreciation_rate,
                step=0.1,
                fmt="%.2f",
                pct=True,
            )
            tax_dep = _num(
                "Tax dep. rate (%)",
                pf + "tax_dep",
                base_finance.tax_depreciation_rate,
                step=0.1,
                fmt="%.2f",
                pct=True,
            )
