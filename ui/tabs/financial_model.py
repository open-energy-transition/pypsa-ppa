    ...
    *,
    ...
    if key not in st.session_state:
        st.session_state[key] = (
            float(default) * scale if not isinstance(default, int) else default
        )
    kwargs = {}
    if step is not None:
        kwargs["step"] = step
...
def _collect_inputs(
    seed: ProjectFinanceInputs, multi_year: bool
) -> ProjectFinanceInputs:
    ...
    with st.expander("📅 Timing (FID, construction, life)", expanded=False):
        ...
        with cols[2]:
            duration = int(
                _num(
                    "Model duration (yrs)", f + "duration", seed.model_duration, step=1
                )
            )
        with cols[3]:
            life = int(
                _num("Operating life (yrs)", f + "life", seed.operating_life, step=1)
            )
    ...
    with st.expander("💰 Revenue & indexation", expanded=False):
        ...
        with cols[0]:
            tenor = int(
                _num("PPA contract tenor (yrs)", f + "tenor", seed.ppa_tenor, step=1)
            )
        ...
        with cols[2]:
            offset = int(
                _num(
                    "Indexation offset (yrs)",
                    f + "offset",
                    seed.indexation_offset_years,
                    step=1,
                )
            )
            cost_infl = _num(
        ...
        with cols[3]:
            ...
            solar_infl = _num(
                "Solar-hour price infl. (%/yr)",
                f + "solar_infl",
                seed.solar_price_inflation,
                step=0.1,
                fmt="%.2f",
                pct=True,
            )
            nonsolar_infl = _num(
                "Non-solar price infl. (%/yr)",
                f + "nonsolar_infl",
                seed.nonsolar_price_inflation,
                step=0.1,
                fmt="%.2f",
                pct=True,
            )
    ...
    with st.expander("🏦 Debt, depreciation & tax", expanded=True):
        ...
        with cols[0]:
            st.markdown("**Debt**")
            debt_tenor = int(
                _num("Repayment tenor (yrs)", f + "debt_tenor", seed.debt_tenor, step=1)
            )
            debt_rate = _num(
                "Debt rate (%)",
                f + "debt_rate",
        ...
        with cols[3]:
            ...
            book_dep = _num(
                "Book depreciation (%/yr)",
                f + "book_dep",
                seed.book_depreciation_rate,
                step=0.1,
                fmt="%.2f",
                pct=True,
            )
    ...
    return ProjectFinanceInputs(
        onsw_build_cost=onsw_build,
        pv_build_cost=pv_build,
        bess_build_cost=bess_build,
        onsw_connection_cost=onsw_conn,
        pv_connection_cost=pv_conn,
        bess_connection_cost=bess_conn,
        onsw_devex=onsw_devex,
        pv_devex=pv_devex,
        bess_devex=bess_devex,
        onsw_fixed_om=onsw_om,
        pv_fixed_om=pv_om,
        bess_fixed_om=bess_om,
        ancillary_pct=anc,
        model_duration=duration,
        fid_period=fid_period,
        onsw_constr_years=onsw_con,
        pv_constr_years=pv_con,
        bess_constr_years=bess_con,
        operating_life=life,
        ppa_tenor=tenor,
        ppa_tariff=tariff,
        penalty_multiple=pen,
        lgc_price=lgc,
        indexation_offset_years=offset,
        cost_inflation=cost_infl,
        ppa_indexation=ppa_idx,
        solar_price_inflation=solar_infl,
        nonsolar_price_inflation=nonsolar_infl,
        escalate_merchant_prices=escalate_merchant,
        debt_tenor=debt_tenor,
        debt_rate=debt_rate,
        dscr_contracted=dscr_c,
        dscr_uncontracted=dscr_u,
        max_gearing_contracted=gear_c,
        max_gearing_uncontracted=gear_u,
        book_depreciation_rate=book_dep,
        tax_depreciation_rate=tax_dep,
...
def _render_results(r) -> None:
    with st.expander("**Key results**", expanded=True):
        ...
        cols[1].metric(
            "Debt / Equity", f"€{r.total_debt:,.0f}m / €{r.total_equity:,.0f}m"
        )
    ...
    with st.expander("**Annual results**", expanded=True):
        ...
        with tab_chart2:
            ...
            fig.add_trace(
                go.Bar(
                    x=periods[ops],
                    y=sc["net_uncontracted_rev"][ops],
                    name="Uncontracted (merchant + LGC)",
                    marker_color="#90CAF9",
                )
            )
            fig.update_layout(
...
def render() -> None:
    ...
    with st.expander("⚡ Energy inputs from PyPSA (pre-filled)", expanded=False):
        ...
        cols = st.columns(4)
        with cols[0]:
            st.metric("PPA delivered", f"{energy.ppa_gwh:,.0f} GWh")
            st.metric("Penalty volume", f"{energy.penalty_gwh:,.0f} GWh")

        with cols[1]:
            st.metric(
                "Total gen (solar / non-solar)",
                f"{energy.total_solar_gwh:,.0f} / {energy.total_nonsolar_gwh:,.0f} GWh",
            )
            st.metric("Capacity (Wind)", f"{energy.onsw_mw:,.0f} MW")

        with cols[2]:
            st.metric(
                "Excess sold (solar / non-solar)",
                f"{energy.excess_solar_gwh:,.0f} / {energy.excess_nonsolar_gwh:,.0f} GWh",
            )
            st.metric("Capacity (PV)", f"{energy.pv_mw:,.0f} MW")

        with cols[3]:
            st.metric(
                "Merchant capture (solar / non-solar)",
    ...
    if result is None:
        st.info(
            "Set your assumptions above and click **Run financial model**.", icon="▶️"
        )
        return
    ...
    with st.expander("⚡ Export financial model as Excel(R) file", expanded=False):
        ...
        try:
            xlsx = export_financial_model(
                result.inputs, result.energy, result, year_results=results_list
            )
