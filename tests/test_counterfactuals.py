...
def test_compute_counterfactuals_fully_hedged_cal_cost_matches_forward_price(
    solved_result, tiny_ts
):
...
def test_compute_counterfactuals_zero_hedge_fraction_equals_spot(
    solved_result, tiny_ts
):
...
def test_compute_counterfactuals_savings_are_internally_consistent(
    solved_result, tiny_ts
):
    result = compute_counterfactuals(tiny_ts, solved_result.scenario, solved_result)
    assert result.ppa_saving_vs_spot == pytest.approx(
        result.spot_cost - result.ppa_offtaker_cost
    )
    assert result.ppa_saving_vs_blended == pytest.approx(
        result.blended_cost - result.ppa_offtaker_cost
    )


def test_compute_counterfactuals_cumulative_series_end_at_total_cost(
    solved_result, tiny_ts
):
    result = compute_counterfactuals(tiny_ts, solved_result.scenario, solved_result)
...
def test_compute_counterfactuals_effective_prices_are_cost_over_volume(
    solved_result, tiny_ts
):
    result = compute_counterfactuals(tiny_ts, solved_result.scenario, solved_result)
    assert result.spot_avg_price == pytest.approx(
        result.spot_cost / result.total_load_mwh
    )
