...
def _wait_for_health(port: int, proc: subprocess.Popen, timeout_s: float) -> requests.Response:
    deadline = time.monotonic() + timeout_s
    last_exc = None
    while time.monotonic() < deadline:
        if proc.poll() is not None:
            output = proc.stdout.read() if proc.stdout else ""
            pytest.fail(f"streamlit process exited early (code {proc.returncode}):\n{output}")
        try:
            return requests.get(f"http://127.0.0.1:{port}/_stcore/health", timeout=2)
        except requests.exceptions.RequestException as exc:
            last_exc = exc
            time.sleep(POLL_INTERVAL_S)
    raise AssertionError(f"app never became healthy within {timeout_s}s (last error: {last_exc})")
