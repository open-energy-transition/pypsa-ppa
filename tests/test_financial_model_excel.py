...
from ppa.financial_model import ProjectFinanceInputs, run_project_finance
...
def test_export_financial_model_includes_hourly_sheets_when_year_results_given(
    ...
    result = run_project_finance(pf_inputs, energy_inputs)
    data = export_financial_model(pf_inputs, energy_inputs, result, year_results=[solved_result])
    ...
    assert wb.sheetnames[-1] in hourly_sheets or wb.sheetnames[-len(hourly_sheets):] == hourly_sheets
...
def test_export_financial_model_outputs_sheet_has_key_kpis(energy_inputs, pf_inputs):
    ...
    all_text = " ".join(
        str(cell.value) for row in outputs_ws.iter_rows() for cell in row if cell.value is not None
    )
